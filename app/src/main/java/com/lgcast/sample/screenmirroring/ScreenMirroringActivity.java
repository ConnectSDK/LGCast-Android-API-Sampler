...
public class ScreenMirroringActivity extends AppCompatActivity {
    ...
    public void onBackPressed() {
        if (ScreenMirroringHelper.isRunning(this)) CommUtil.showDialog(this, false, "Notice", "Screen mirroring is in process.\nStop?", index -> finish(), index -> Log.v(TAG, ""));
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_CODE_CAPTURE_PERMISSION) {
            ...
            else Toast.makeText(this, "Permission is required to connect to TV.", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void onClickStartMirroring(View v) {
        if (ScreenMirroringHelper.isRunning(this)) {
            Toast.makeText(this, "Screen mirroring is already started.", Toast.LENGTH_SHORT).show();
        } else {
            if (hasCapturePermission()) chooseDevice();
    ...
    public void onClickStopMirroring(View v) {
        ...
        else Toast.makeText(this, "Screen mirroring is not started.", Toast.LENGTH_SHORT).show();
    }
    ...
    private void startMirroring(Intent projectionData) {
        ...
        progress.setMessage("Connecting to TV...");
        progress.show();

        AlertDialog pairingAlert = new AlertDialog.Builder(this)
                .setTitle("Notification")
                .setCancelable(false)
                .setMessage("Please accept pairing request on TV.")
        ...
        mWebOSTVService.startScreenMirroring(this, projectionData, new LGCastControl.ScreenMirroringStartListener() {
            ...
            public void onSuccess(SecondScreen secondScreen) {
                Toast.makeText(ScreenMirroringActivity.this, "Screen mirroring is Started.", Toast.LENGTH_LONG).show();
                updateButtonVisibility();
            ...
            public void onError(ServiceCommandError error) {
                Toast.makeText(ScreenMirroringActivity.this, "Failed to start Screen mirroring.", Toast.LENGTH_LONG).show();
    ...
    private void stopMirroring() {
        ...
        ProgressDialog progress = new ProgressDialog(this);
        progress.setMessage("Disconnecting TV...");
        progress.show();

        mWebOSTVService.stopScreenMirroring(this, new LGCastControl.ScreenMirroringStopListener() {
            @Override
            public void onSuccess(String message) {
                Toast.makeText(ScreenMirroringActivity.this, "Stopped Screen mirroring.", Toast.LENGTH_LONG).show();
                updateButtonVisibility();
                progress.dismiss();
            }
